...
describe('FilterChipBar', () => {
  ...
  // ---- Spec 4.4 — Quick-add chip toggles the canonical condition ----
  // Click adds the condition; click again removes it. Active state is signalled
  // via aria-pressed (not `disabled`) per the 2026-05-27 toggle-behavior change.
  it('quick-add chip toggles the canonical condition on/off', () => {
    const onChange = vi.fn();
    const { rerender } = render(<QuickAddChips value={null} onChange={onChange} />);

    // First click: adds the condition.
    fireEvent.click(screen.getByTestId('quick-add-offline'));
    expect(onChange).toHaveBeenCalled();
    const afterAdd = onChange.mock.calls[0][0] as FilterConditionGroup;
    expect(afterAdd.conditions[0]).toMatchObject({ field: 'status', operator: 'equals', value: 'offline' });

    // Re-render with the new group; button should now reflect "pressed" state.
    rerender(<QuickAddChips value={afterAdd} onChange={onChange} />);
    expect(screen.getByTestId('quick-add-offline')).toHaveAttribute('aria-pressed', 'true');

    // Second click: removes the condition (toggle off).
    fireEvent.click(screen.getByTestId('quick-add-offline'));
    const afterRemove = onChange.mock.calls[onChange.mock.calls.length - 1][0] as FilterConditionGroup;
    expect(afterRemove.conditions).toEqual([]);
  });
